...
watchEffect(() => {
  if (!props.placeholder) {
    if (asRange()) {
      givenPlaceholder.value = `${props.displayFormatter.date}${props.separator}${props.displayFormatter.date}`
    } else {
      givenPlaceholder.value = props.displayFormatter.date
    }
  } else {
...
const displayDateTime = (pickerValue)=> {
  ...
  if(pickerValue) {
    if (asRange()) {
      ...
      if (sd.isValid() && ed.isValid()) {
        ...
        let displayDate = dayjs(s, props.formatter.date, true).format(props.displayFormatter.date.replaceAll('Y', (props.buddhistEnable ? 'B' : 'Y'))) + "" + props.separator + "" + dayjs(e, props.formatter.date, true).format(props.displayFormatter.date.replaceAll('Y', (props.buddhistEnable ? 'B' : 'Y')))
        if(props.locale === "th") {
          displayDate = displayDate.replaceAll("January", "มกราคม")
                            .replaceAll("Jan", "ม.ค.")
                            .replaceAll("February", "กุมภาพันธ์")
                            .replaceAll("Feb", "ก.พ.")
                            .replaceAll("March", "มีนาคม")
                            .replaceAll("Mar", "มี.ค.")
                            .replaceAll("April", "เมษายน")
                            .replaceAll("Apr", "เม.ย.")
                            .replaceAll("May", "พฤษภาคม")
                            .replaceAll("June", "มิถุนายน")
                            .replaceAll("Jun", "มิ.ย.")
                            .replaceAll("July", "กรกฎาคม")
                            .replaceAll("Jul", "ก.ค.")
                            .replaceAll("August", "สิงหาคม")
                            .replaceAll("Aug", "ส.ค.")
                            .replaceAll("September", "กันยายน")
                            .replaceAll("Sep", "ก.ย.")
                            .replaceAll("October", "ตุลาคม")
                            .replaceAll("Oct", "ต.ค.")
                            .replaceAll("November", "พฤศจิกายน")
                            .replaceAll("Nov", "พ.ย.")
                            .replaceAll("December", "ธันวาคม")
                            .replaceAll("Dec", "ธ.ค.")
        }
        return displayDate 
      }
    } else {
      const d = dayjs(pickerValue, props.formatter.date, true)
      if (d.isValid()) {
        dayjs.locale(props.locale)
        let displayDate = dayjs(pickerValue, props.formatter.date, true).format(props.displayFormatter.date.replaceAll('Y', (props.buddhistEnable ? 'B' : 'Y')))
        if(props.locale === "th") {
          displayDate = displayDate.replaceAll("January", "มกราคม")
                            .replaceAll("Jan", "ม.ค.")
                            .replaceAll("February", "กุมภาพันธ์")
  ...
  return pickerValue === "" ? givenPlaceholder.value : pickerValue
}
</script>
    ...
    <PopoverButton as="label" class="relative block">
      <slot :value="pickerValue" :placeholder="givenPlaceholder" :clear="clearPicker">
        <input
          ...
          @keyup="keyUp"
          :value="displayDateTime(pickerValue)"
        />
        <div
            ...
            >
              <div class="flex flex-wrap lg:flex-nowrap">
                ...
                <div class="relative flex flex-wrap sm:flex-nowrap p-1 w-full">
                  ...
                  >
                    <vtd-header :panel="panel.previous" :calendar="calendar.previous" :buddhist-enable="props.buddhistEnable" />
                    <div class="px-0.5 sm:px-2">
                      <vtd-month
                        v-show="panel.previous.month"
                        :months="months"
                        @update:month="calendar.previous.setMount"
                        :buddhist-enable="props.buddhistEnable"
                      />
                      <vtd-year
                      ...
                      <div v-show="panel.previous.calendar">
                        <vtd-week :weeks="weeks" :buddhist-enable="props.buddhistEnable" />
                        <vtd-calendar
                  ...
                  >
                    <vtd-header as-prev-or-next :panel="panel.next" :calendar="calendar.next" :buddhist-enable="props.buddhistEnable" />
                    <div class="px-0.5 sm:px-2">
                      <vtd-month v-show="panel.next.month" :months="months" @update:month="calendar.next.setMount" :buddhist-enable="props.buddhistEnable" />
                      <vtd-year
                        as-prev-or-next
      ...
      <div class="flex flex-wrap lg:flex-nowrap">
        ...
        <div class="relative flex flex-wrap sm:flex-nowrap p-1 w-full">
          ...
          >
            <vtd-header :panel="panel.previous" :calendar="calendar.previous" :buddhist-enable="props.buddhistEnable" />
            <div class="px-0.5 sm:px-2">
              <vtd-month v-show="panel.previous.month" :months="months" @update:month="calendar.previous.setMount" :buddhist-enable="props.buddhistEnable" />
              <vtd-year
                v-show="panel.previous.year"
                :years="calendar.previous.years()"
                @update:year="calendar.previous.setYear"
